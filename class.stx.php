...
class STX
{
	...
	var $status;
	var $stx_path = '';  //目录
	var $lang_arr = array();  //语言数组
	var $ext_arr = array(	  //扩展语言
				'cpp'=>'cpp',
				'h'=>'cpp',
				'c'=>'cpp',
				'cs'=>'cs',
	...
	function set_stx()
	{
		...
		foreach ($this->lang_arr as $lang) {
			...
			$line_arr = file($stx_file);  //规则
			foreach ($line_arr as $val) 
			{
				$val = trim($val);
				if (strlen($val) == 0) continue;  //空行
	...
	function stx_string($str)
	{
		...
		$words_begin = '<span style="color:'.$this->color['words'].';font-family:Courier New;"  class="begin-color">';
		...
		$functions_begin = '<span class="functions">';
		...
		for($i=0;$i<count($rs_arr);$i++)
		{
			$j = $i+1;
			if(!$is_com)
				//$is_has = $is_com = preg_match('<span class="pl-c">',$rs_arr[$i]);
				$is_has = $is_com = (strpos($rs_arr[$i],'<span class="pl-c">'));
			if($is_has&&$is_com)
			{
				$tmp .= '<tr><td id="LC'.$j.'"   class="blob-num">'.($i+1) .':</td><td  class="blob-code-inner" ><span class="pl-c">'. $rs_arr[$i] ."</span></td></tr>" ;
				$is_has = 0;
				if(preg_match('</span>',$rs_arr[$i]))
				{
					$is_com = 0;
				}
			}	
			elseif($is_com &&!preg_match('</span>',$rs_arr[$i]))
			{
				$tmp .=  '<tr><td id="LC'.$j.'"  class="blob-num" >'.($i+1) .':</td><td class="blob-code-inner" ><span class="pl-c">'. $rs_arr[$i] ."</span></td></tr>" ;
			}elseif($is_com &&preg_match('</span>',$rs_arr[$i]))
			{
				...
				$tmp .=  '<tr><td id="LC'.$j.'" class="blob-num" >'.($i+1) .':</td><td class="blob-code-inner" >'. $rs_arr[$i] ."</td></tr>" ;
